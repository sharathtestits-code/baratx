# Cursor chat records

Durable handoffs from Cursor Cloud Agent chats for **BarathX** (TestITS / Sharath).

**Why this folder exists:** Cursor chat history does **not** transfer when you switch Cursor accounts. Committing handoffs here keeps strategy, PR checklists, and ops context alive via GitHub.

**How to use:** Open the latest dated `*-handoff.md` before starting a new agent session. Prefer these docs + open PRs over relying on old Cloud Agent UI history.

**Repo:** https://github.com/sharathtestits-code/baratx  
**Site:** https://barathx.com · Social: [@getbaratx](https://x.com/getbaratx)
